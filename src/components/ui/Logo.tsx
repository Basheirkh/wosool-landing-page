...
export default function Logo({
  ...
  const fill = color === "white" ? "#F5F5F5" : color === "brand" ? "#00D97E" : "#111827";
  const textColor =
    color === "white" ? "text-white" : color === "brand" ? "text-brand-primary" : "text-[#111827]";
  ...
  return (
    <a href="#" className={`flex items-center gap-2 ${className}`}>
      ...
      {showText && (
        <span className={`${s.text} font-semibold ${textColor}`}>وصول</span>
      )}
    </a>
  );
}
